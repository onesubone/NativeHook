...
#ifndef RUBICK_ELF_DL_HOOK_H
#define RUBICK_ELF_DL_HOOK_H
...
typedef struct {
     ...
     */
    Elf32_Word st_name;      // 符号名。如果该值非0，则表示符号名的字符串表索引(offset，即偏移地址)，否则符号表项没有名称。注:外部 C 符号在 C 语言和目标文件的符号表中具有相同的名称。
    /**
